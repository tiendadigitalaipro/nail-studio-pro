...
const categoryIcons: Record<string, React.ReactNode> = {
  ...
  'Step': <Hash className="h-3.5 w-3.5" />,
...
export function MarketSelector() {
  ...
  const handleSelect = (symbol: string) => {
    if (isConnected) subscribeToMarket(symbol);
  };
                ...
                {items.map((market) => {
                  ...
                  return (
                    <button
                      ...
                      `}
                      title={`${market.description} | Symbol: ${market.symbol} | Contracts: ${market.supportedContracts.slice(0, 4).join(', ')}`}
                    >
